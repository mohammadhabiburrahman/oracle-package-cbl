CREATE OR REPLACE PACKAGE npsb_xml_pck IS
	...
	PROCEDURE app_npstransactions_table_d(p_file_name VARCHAR2);
...
CREATE OR REPLACE PACKAGE BODY npsb_xml_pck IS
	...
	PROCEDURE app_npstransactions_table_d(p_file_name VARCHAR2) IS
	BEGIN
		DELETE FROM app_npstransactions t WHERE t.filename = p_file_name;
		COMMIT;
	END;
END;
/
