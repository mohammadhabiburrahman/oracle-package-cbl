CREATE OR REPLACE PACKAGE custom_account IS
    ...
    */
	...
	FUNCTION get_last_positive_bal_doc_bdt(p_con VARCHAR2) RETURN NUMBER;
	FUNCTION get_last_positive_bal_doc_usd(p_con VARCHAR2) RETURN NUMBER;
	FUNCTION get_last_pstv_bal_doc_bdt_woff(p_con VARCHAR2) RETURN NUMBER;
	FUNCTION get_last_pstv_bal_doc_usd_woff(p_con VARCHAR2) RETURN NUMBER;
...
CREATE OR REPLACE PACKAGE BODY custom_account IS
    ...
    */
			...
			BEGIN
				...
				FROM   (SELECT t.docno
							  ...
							  ,SUM(CASE
									   ...
									   WHEN t.trantype IN (1, 2) THEN
										-t.amount
								   END) over(ORDER BY t.docno ASC) cum_sum
						FROM   a4m.tcontracttrxnlist t
						JOIN   a4m.tcontractitem i
						ON     i.no = t.contractno
						AND    i.key = t.accountno
	...
	FUNCTION get_lst_pval_doc_ftran_chk(p_con VARCHAR2) RETURN NUMBER IS
		v_lst_pval_doc     NUMBER := NULL;
	...
	BEGIN
		BEGIN
			v_lst_pval_doc := get_last_positive_bal_doc(p_con);
		EXCEPTION
			WHEN OTHERS THEN
				v_lst_pval_doc := NULL;
				NULL;
		...
		IF v_lst_pval_doc IS NOT NULL
		   AND v_lst_pval_doc = 01061991
		THEN
			v_lst_pval_doc_bdt := get_last_positive_bal_doc_bdt(p_con);
			v_lst_pval_doc_usd := get_last_positive_bal_doc_usd(p_con);
			v_bdt_account      := get_bdt_acc_by_con(p_con);
			v_usd_account      := get_usd_acc_by_con(p_con);
			...
			BEGIN
				...
				WHERE  t.branch = 1
				AND    t.contractno = p_con
				AND    t.docno = v_lst_pval_doc_bdt
				AND    t.accountno = v_bdt_account
				AND    t.debitentcode IN
					  -- (999, 1009, 1011, 1012, 1013, 1000, 1003, 1004, 1005, 1006, 1007, 998);
					   (21
			...
			ELSE
				BEGIN
					...
					WHERE  t.branch = 1
					AND    t.contractno = p_con
					AND    t.docno = v_lst_pval_doc_usd
					AND    t.accountno = v_usd_account
					AND    t.debitentcode IN
						  --   (999, 1009, 1011, 1012, 1013, 1000, 1003, 1004, 1005, 1006, 1007, 998);
						   (21
	...
	FUNCTION get_last_positive_bal_doc_bdt(p_con VARCHAR2) RETURN NUMBER IS
		v_docno       NUMBER := NULL;
		v_bdt_account VARCHAR2(30) := NULL;
		--v_usd_account                   VARCHAR2(30) := NULL;
		v_global_limit_migrated_account VARCHAR2(30) := NULL;
	...
	FUNCTION get_last_positive_bal_doc_usd(p_con VARCHAR2) RETURN NUMBER IS
		v_docno NUMBER := NULL;
		--v_bdt_account                   VARCHAR2(30) := NULL;
		v_usd_account                   VARCHAR2(30) := NULL;
		v_global_limit_migrated_account VARCHAR2(30) := NULL;
	...
	FUNCTION get_last_pstv_bal_doc_bdt_woff(p_con VARCHAR2) RETURN NUMBER IS
		v_docno       NUMBER := NULL;
		v_bdt_account VARCHAR2(30) := NULL;
		--v_usd_account                   VARCHAR2(30) := NULL;
		v_global_limit_migrated_account VARCHAR2(30) := NULL;
	...
	FUNCTION get_last_pstv_bal_doc_usd_woff(p_con VARCHAR2) RETURN NUMBER IS
		v_docno NUMBER := NULL;
		--v_bdt_account                   VARCHAR2(30) := NULL;
		v_usd_account                   VARCHAR2(30) := NULL;
		v_global_limit_migrated_account VARCHAR2(30) := NULL;
	...
	BEGIN
		...
		THEN
			...
			IF v_lst_pval_doc_bdt IS NULL
			   AND v_lst_pval_doc_usd IS NULL
			THEN
				v_flag := 0;
				RETURN v_flag;
			END IF;
		
			IF v_lst_pval_doc_bdt IS NOT NULL
			THEN
				BEGIN
			...
			THEN
				...
				RETURN v_flag;
			ELSE
				IF v_lst_pval_doc_usd IS NOT NULL
				THEN
					BEGIN
