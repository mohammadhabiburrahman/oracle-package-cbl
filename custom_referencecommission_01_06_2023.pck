...
CREATE OR REPLACE PACKAGE BODY custom_referencecommission AS
	...
	BEGIN
		...
		THEN
			...
			vstring := datamember.getintervalchar(object_type
	...
	BEGIN
		...
		vret := datamember.datamemberdelete(object_type, scommissionrow.commission, 'INTERVAL');
	...
	BEGIN
		sintreccount := 0;
		LOOP
			vinterval := datamember.getintervalnext(object_type, pcommission, 'INTERVAL', vinterval);
			EXIT WHEN vinterval IS NULL;
			vstring := datamember.getintervalchar(object_type, pcommission, 'INTERVAL', vinterval);
			sintreccount := sintreccount + 1;
	...
	BEGIN
		...
		vret := datamember.datamemberdelete(object_type
										   ,dialog.getnumber(pdialog, 'Commission')
										   ,'INTERVAL');
		FOR i IN 1 .. sintreccount
		LOOP
			vret := datamember.setintervalchar(object_type
											  ,dialog.getnumber(pdialog, 'Commission')
											  ,'INTERVAL'
											  ,saintint(i)
	...
	FUNCTION getdata(pident IN VARCHAR) RETURN NUMBER IS
		...
		vbranch NUMBER := seance.getbranch;
	BEGIN
		vret := datamember.getnumber(object_type, vbranch, pident);
		RETURN vret;
	END;
	...
	(
	   ...
	   ,pvalue IN NUMBER
		...
		vbranch NUMBER := seance.getbranch;
	BEGIN
		vret := datamember.setnumber(object_type, vbranch, pident, pvalue, NULL);
	END;
