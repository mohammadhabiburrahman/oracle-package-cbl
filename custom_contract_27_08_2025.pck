CREATE OR REPLACE PACKAGE custom_contract IS
    ...
    */
	...
	FUNCTION get_instl_acc_list(p_contract_no VARCHAR2) RETURN VARCHAR2;
END;
/
CREATE OR REPLACE PACKAGE BODY custom_contract IS
	...
	FUNCTION get_instl_acc_list(p_contract_no VARCHAR2) RETURN VARCHAR2 IS
		v_acc_list VARCHAR2(500);
	BEGIN
		BEGIN
			SELECT listagg(l.linkno, ', ') within GROUP(ORDER BY l.linkno)
			INTO   v_acc_list
