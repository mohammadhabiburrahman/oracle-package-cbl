CREATE OR REPLACE PACKAGE custom_transaction IS
	...
	FUNCTION get_multiple_trns_value
	(
		p_docno     NUMBER
	   ,result_type VARCHAR2
	) RETURN NUMBER;
	FUNCTION get_ttl_spending_contract
	(
		p_contract_no VARCHAR2
	   ,p_fromdate    DATE
	   ,p_to_date     DATE
	) RETURN NUMBER;
	FUNCTION get_ttl_payment_con
	(
		p_con      VARCHAR2
	   ,p_frm_date DATE
	   ,p_to_date  DATE
	) RETURN NUMBER;
	FUNCTION get_usd_spending_contract
	(
		p_contract_no VARCHAR2
	   ,p_fromdate    DATE
	   ,p_to_date     DATE
	) RETURN NUMBER;
	FUNCTION get_bdt_spending_contract
	(
		p_contract_no VARCHAR2
	   ,p_fromdate    DATE
	   ,p_to_date     DATE
	) RETURN NUMBER;
	FUNCTION get_installment_os_spec_date_bb(p_contract_no VARCHAR2
											 -- ,p_fromdate    DATE
											,p_to_date DATE) RETURN NUMBER;
...
CREATE OR REPLACE PACKAGE BODY custom_transaction IS
	...
	BEGIN
		BEGIN
			SELECT x.account_no
				  ,SUM(acc_info.remain) AS installment_amount
			INTO   v_acc_no
				  ,v_installment_amount
			FROM   (SELECT ci.branch
						  ,ci.no AS contract_no
						  ,ci.key AS account_no
						  ,cl.mainno
						  ,cl.linkno
						  ,cl.linkname
						  ,(SELECT key FROM a4m.tcontractitem WHERE no = cl.linkno) AS installment_account_no
			...
			JOIN   a4m.taccount acc_info
			ON     x.branch = acc_info.branch
	...
	BEGIN
		BEGIN
			...
			AND    e.debitentcode IN (63, 98, 203, 33)
			AND    t.docno = p_docno;
		EXCEPTION
			WHEN OTHERS THEN
				v_is_ecom := 0;
	...
	BEGIN
		BEGIN
			...
			IF v_is_ecom IN (8, 89, 88, 87, 82, 81, 66, 65, 62, 61, 59, 53, 52, 78) --ECOM
			THEN
				v_is_ecom := 1;
	...
	BEGIN
		BEGIN
			...
			ELSIF v_is_ecom IS NULL
				  AND v_pos_condtion IN (8, 88, 87, 82, 81, 66, 65, 64, 62, 61, 60, 59, 53, 52, 78) --ECOM
			THEN
				v_is_ecom := 1;
			...
			THEN
				...
				END IF;
				IF v_is_ecom IS NULL
				   AND v_pos_condtion IN (0, 1, 2, 3, 4, 5, 7, 90, 91, 95, 67, 10) --POS
				---Pos Condition 10 added on 04-12-2024 as a suspected POS transaction by Himel and Jubayer vai
				THEN
					v_is_ecom := 0; --POS
					RETURN v_is_ecom;
				END IF;
	...
	BEGIN
		BEGIN
			...
			FROM   (SELECT t.value amt
					...
					WHERE  t.docno = p_doc
					AND    t.branch = 1
					AND    t.debitentcode NOT IN (63, 98, 805, 33)
					ORDER  BY t.no DESC)
			WHERE  rownum = 1;
		EXCEPTION
			WHEN OTHERS THEN
	...
	BEGIN
		...
		BEGIN
			...
			LEFT   JOIN a4m.tcard c
			ON     c.pan = t.pan
			AND    c.branch = t.branch
			
			LEFT   JOIN a4m.tatmext p
			ON     p.docno = e.docno
			AND    p.branch = e.branch
			
			LEFT   JOIN tla@cms2two o
			ON     o.pan = c.pan
			AND    o.approvalcode = p.approval
			AND    o.trancode <> 111
			
			LEFT   JOIN a4m.tatmext a
			ON     a.docno = e.docno
	...
	(
	   ...
	   ,result_type VARCHAR2
		...
		v_settlement_amt_usd NUMBER := 0;
		v_settlement_amt_bdt NUMBER := 0;
		v_incomewithvat_usd  NUMBER := 0;
		v_vat_usd            NUMBER := 0;
		v_income_usd         NUMBER := 0;
		v_incomewithvat_bdt  NUMBER := 0;
		v_vat_bdt            NUMBER := 0;
		v_income_bdt         NUMBER := 0;
	
	BEGIN
		BEGIN
		
			SELECT t.value settlement_amt_usd
				  ,nvl(t.value * custom_contract.get_exchange_rate(d.opdate), 0) settlement_amt_bdt
				  ,nvl(round(custom_transaction.get_usd_by_doc(d.docno) - t.value, 2), 0) incomewithvat
				  ,round(nvl(round(custom_transaction.get_usd_by_doc(d.docno) - t.value, 2), 0) *
						 0.1304347826
						,2) vat
				  ,round(custom_transaction.get_usd_by_doc(d.docno) - t.value, 2) -
				   round(nvl(round(custom_transaction.get_usd_by_doc(d.docno) - t.value, 2), 0) *
						 0.1304347826
						,2) income
				   -- ,t.value * a4m.custom_contract.get_exchange_rate(d.opdate) settlement_amt_bdt
				  ,nvl(round(custom_transaction.get_usd_by_doc(d.docno) - t.value, 2) *
					   custom_contract.get_exchange_rate(d.opdate)
					  ,0) incomewithvat_bdt
				  ,round(nvl(round(custom_transaction.get_usd_by_doc(d.docno) - t.value, 2), 0) *
						 0.1304347826
						,2) * custom_contract.get_exchange_rate(d.opdate) vat_bdt
				  ,nvl((round(custom_transaction.get_usd_by_doc(d.docno) - t.value, 2) *
					   custom_contract.get_exchange_rate(d.opdate)) -
					   (round(nvl(round(custom_transaction.get_usd_by_doc(d.docno) - t.value, 2)
								 ,0) * 0.1304347826
							 ,2) * custom_contract.get_exchange_rate(d.opdate))
					  ,0) income_bdt
			
			INTO   v_settlement_amt_usd
				  ,v_settlement_amt_bdt
				  ,v_incomewithvat_usd
				  ,v_vat_usd
				  ,v_income_usd
				  ,v_incomewithvat_bdt
			...
			IF result_type = 'SETTLE_AMT_USD'
			THEN
				v_return := v_settlement_amt_usd;
			ELSIF result_type = 'SETTLE_AMT_BDT'
			THEN
				v_return := v_settlement_amt_bdt;
			ELSIF result_type = 'INCOME_WITH_VAT_USD'
			THEN
				v_return := v_incomewithvat_usd;
			ELSIF result_type = 'INCOME_WITH_VAT_BDT'
			THEN
				v_return := v_incomewithvat_bdt;
			ELSIF result_type = 'VAT_USD'
			THEN
				v_return := v_vat_usd;
			ELSIF result_type = 'VAT_BDT'
			THEN
				v_return := v_vat_bdt;
			ELSIF result_type = 'INCOME_USD'
			THEN
				v_return := v_income_usd;
			ELSIF result_type = 'INCOME_BDT'
			THEN
				v_return := v_income_bdt;
			ELSE
				v_return := 0;
			END IF;
		EXCEPTION
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				SELECT SUM(nvl(get_bdt_by_doc(a.docno), 0))
				INTO   v_credit_value
				FROM   tentry    a
					  ,tdocument b
				WHERE  a.branch = 1
				...
				AND    b.docno = a.docno
				AND    b.newdocno IS NULL
				AND    --(a.debitentcode IN (104) OR
					   a.debitentcode IN
					   (SELECT entcode FROM a4m.tcontractentrygrouplist WHERE groupid IN (1, 2, 14)) --)
				AND    a.creditaccount = v_account_no_bdt
				AND    (b.opdate BETWEEN p_fromdate AND p_to_date OR
					  (p_fromdate IS NULL AND p_to_date IS NULL));
			EXCEPTION
			...
			THEN
				...
				SELECT nvl(SUM(nvl(get_bdt_by_doc(a.docno), 0)), 0)
				INTO   v_credit_value_usd
				FROM   tentry    a
					  ,tdocument b
				WHERE  a.branch = 1
	...
	BEGIN
		...
		BEGIN
			...
			AND    t.postdate BETWEEN p_frm_date AND p_to_date;
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				SELECT SUM(nvl(get_usd_by_doc(a.docno), 0))
				INTO   v_credit_value
				FROM   tentry    a
					  ,tdocument b
				WHERE  a.branch = 1
				...
				AND    b.docno = a.docno
				AND    b.newdocno IS NULL
				AND    get_currency_by_doc(b.docno) = 840
				AND    --(a.debitentcode IN (104) OR
					   a.debitentcode IN
					   (SELECT entcode FROM a4m.tcontractentrygrouplist WHERE groupid IN (1, 2, 14)) --)
				AND    a.creditaccount = v_account_no_bdt
			...
			THEN
				...
				SELECT nvl(SUM(nvl(get_usd_by_doc(a.docno), 0)), 0)
				INTO   v_credit_value_usd
				FROM   tentry    a
					  ,tdocument b
				WHERE  a.branch = 1
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				AND    b.docno = a.docno
				AND    b.newdocno IS NULL
				AND    get_currency_by_doc(b.docno) = 50
				AND    --(a.debitentcode IN (104) OR
					   a.debitentcode IN
					   (SELECT entcode FROM a4m.tcontractentrygrouplist WHERE groupid IN (1, 2, 14)) --)
				AND    a.creditaccount = v_account_no_bdt
	...
	BEGIN
		...
		BEGIN
			...
			THEN
				v_install_balance_bdt := 0;
			END IF;
			v_balance_bdt := v_install_balance_bdt;
		EXCEPTION
			WHEN OTHERS THEN
				v_result := 0;
				RETURN v_result;
