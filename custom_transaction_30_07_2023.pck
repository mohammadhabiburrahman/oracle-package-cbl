...
CREATE OR REPLACE PACKAGE BODY custom_transaction IS
	...
	BEGIN
		BEGIN
			...
			AND    t.poscondition NOT IN (0, 91)
	...
	BEGIN
		BEGIN
			...
			AND    e.branch = t.branch
			WHERE  t.posentrymode IN (10, 11, 12)
			AND    t.poscondition IN (08, 59, 61, 62, 65, 66, 81, 82, 87, 88, 89)
			AND    e.debitentcode IN (63, 98, 203)
			AND    t.docno = p_docno;
		EXCEPTION
			WHEN OTHERS THEN
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				IF v_install_acc_bdt IS NOT NULL
				THEN
					BEGIN
						SELECT SUM(nvl(a.value, 0))
						...
						AND    b.docno = a.docno
						AND    b.newdocno IS NULL
						AND    a.creditaccount = v_install_acc_bdt
						AND    (b.opdate <= p_to_date OR p_to_date IS NULL);
					EXCEPTION
						WHEN OTHERS THEN
							NULL;
					...
					BEGIN
						...
						AND    b.docno = a.docno
						AND    b.newdocno IS NULL
						AND    a.debitaccount = v_install_acc_bdt
						AND    (b.opdate <= p_to_date OR p_to_date IS NULL);
					EXCEPTION
						WHEN OTHERS THEN
							NULL;
					END;
					v_install_balance_bdt := nvl(v_install_debit_value, 0) -
