...
CREATE OR REPLACE PACKAGE BODY custom_contract IS
	...
	(
	   ...
	   ,p_currency NUMBER
		...
		--result_v_amnt      VARCHAR2(50) := '';
		--result_v_amount    NUMBER := 0;
		--result_v_count     NUMBER := 0;
		--result_v_row_count NUMBER := 0;
	...
	BEGIN
		...
		LOOP
			...
			THEN
				is_seq_found := 1;
				IF (v_row_count = 0 AND v_amount = 0)
				THEN
					v_row_count := i.diff;
					v_amount    := i.amount;
				ELSE
				...
				END IF;
				--RETURN v_row_count;
			ELSIF (i.diff > 1 AND is_seq_found = 0)
			THEN
			
				v_row_count := 1;
				v_amount    := i.amount;
			
				/*result_v_amnt      := to_char(v_amount);
                result_v_amount    := 0;
                result_v_count     := v_row_count;
                result_v_row_count := 0;
