...
CREATE OR REPLACE PACKAGE BODY custom_contract IS
	...
	BEGIN
        ...
        */
		...
		BEGIN
			SELECT CASE
					   WHEN csmpd.mp_payoffdate_atcycleend IS NULL THEN
						csmpd.minpayment
					   WHEN csmpd.mp_payoffdate_atcycleend IS NOT NULL
							AND csmpd.mp_payoffdate_atcycleend <= in_bydate THEN
						CASE
							WHEN (csmpd.minpayment - csmpd.repayment_atcycleend) > 0 THEN
							 (csmpd.minpayment - csmpd.repayment_atcycleend)
							ELSE
							 0
						END
					   ELSE
			...
			AND    ci.no = in_contractno --700000150121
			AND    ci.itemtype = 1
			...
			AND    csc.lastduedate <= in_bydate --'20-SEP-2023'
