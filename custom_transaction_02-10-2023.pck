CREATE OR REPLACE PACKAGE custom_transaction IS
  FUNCTION get_sum_tras_amt_frm_contract(p_contract_no VARCHAR2,
                                         p_fromdate    DATE,
  ...
  FUNCTION get_pur_trns_amt_contract(p_contract_no VARCHAR2,
                                     p_fromdate    DATE,
                                     p_to_date     DATE) RETURN NUMBER;
  ...
  FUNCTION get_cash_w_amt_contract(p_contract_no VARCHAR2,
                                   p_fromdate    DATE,
                                   p_to_date     DATE) RETURN NUMBER;

  FUNCTION get_cash_usage_for_coll(p_contract_no VARCHAR2,
                                   p_fromdate    DATE,
                                   p_to_date     DATE) RETURN NUMBER;
  FUNCTION get_pur_usage_for_coll(p_contract_no VARCHAR2,
                                  p_fromdate    DATE,
                                  p_to_date     DATE) RETURN NUMBER;
  ...
  FUNCTION get_bdt_outstanding_spec_date(p_contract_no VARCHAR2
                                        ...
                                        ,
                                         p_to_date DATE) RETURN NUMBER;
  FUNCTION get_pos_entry_mode(p_docno     NUMBER,
                              p_card_no   VARCHAR2,
                              p_tran_code VARCHAR2 := NULL) RETURN NUMBER;
  FUNCTION get_entcode_by_doc(p_doc NUMBER) RETURN NUMBER;
  FUNCTION get_pos_condition(p_docno     NUMBER,
                             p_card_no   VARCHAR2 := NULL,
                             p_tran_code VARCHAR2 := NULL) RETURN NUMBER;
  FUNCTION is_ecom(p_docno     NUMBER,
                   p_card_no   VARCHAR2 := NULL,
                   p_tran_code VARCHAR2 := NULL) RETURN NUMBER;
  ...
  FUNCTION get_usd_os_spec_date_bb(p_contract_no VARCHAR2
                                  ...
                                  ,
                                   p_to_date DATE) RETURN NUMBER;
  function get_bdt_purchase_by_cid(p_idclient number,
                                   p_frm_date date,
                                   p_to_date  date) return number;
  function get_usd_purchase_by_cid(p_idclient number,
                                   p_frm_date date,
                                   p_to_date  date) return number;
...
CREATE OR REPLACE PACKAGE BODY custom_transaction IS
  ...
  BEGIN
    BEGIN
      SELECT a.key, acc.currencyno
        INTO v_account_no_usd, v_curr
       ...
       WHERE c.type = t.contracttype
         ...
         AND t.itemname IN
             ('ITEMDEPOSITDOM', 'ITEMDEPOSITINT', 'ITEMINSTALLMENT')
  ...
  BEGIN
    BEGIN
      SELECT a.key, acc.currencyno
        INTO v_account_no_mr, v_curr_mr
        FROM tcontracttypeitemname t,
             tcontract             c,
             tcontractitem         a,
             taccount              acc
  ...
  BEGIN
    BEGIN
      WITH acc_info AS
       (SELECT branch, accountno, currencyno, remain FROM a4m.taccount)
      SELECT /*x.BRANCH,x.contract_no,x.MAINNO,x.LINKNO,x.LINKNAME,x.INSTALLMENT_ACCOUNT_NO,*/
       x.account_no, SUM(acc_info.remain) installment_amount
        INTO v_acc_no, v_installment_amount
        FROM (SELECT ci.branch,
                ...
                FROM a4m.tcontractitem ci, a4m.tcontractlink cl
               WHERE ci.branch = 1
      ...
      SELECT amt
        ...
        FROM (SELECT t.value amt
              --INTO   v_amount
               ...
               WHERE t.docno = p_doc
                 AND t.branch = 1
                 AND (t.debitaccount = 20203050007 OR
                     t.creditaccount = 20203050007)
               ORDER BY t.no ASC)
       WHERE rownum = 1;
    EXCEPTION
      ...
      SELECT amt
        ...
        FROM (SELECT t.value amt
              --INTO   v_amount
               ...
               WHERE t.docno = p_doc
                 AND t.branch = 1
                 AND (t.debitaccount = 20203840001 OR
                     t.creditaccount = 20203840001)
               ORDER BY t.no ASC)
       WHERE rownum = 1;
    EXCEPTION
      WHEN no_data_found THEN
      ...
      SELECT DISTINCT 1
       ...
       WHERE t.posentrymode IN (10, 11, 12)
         AND nvl(t.poscondition, 0) IN
             (08, 59, 61, 62, 65, 66, 81, 82, 87, 88, 89)
         AND e.debitentcode IN (63, 98, 203)
         AND t.docno = p_docno;
    EXCEPTION
      WHEN OTHERS THEN
  ...
  BEGIN
    ...
    BEGIN
      ...
      BEGIN
        ...
        FOR z IN install_acc_list --v_install_acc_list.first .. v_install_acc_list.last
         LOOP
          ...
          IF z.accountno IS NOT NULL THEN
            BEGIN
              SELECT SUM(nvl(a.value, 0))
      ...
      END;
      v_install_balance_bdt := nvl(v_install_debit_bal, 0) -
                               nvl(v_install_credit_bal, 0);
      v_balance_bdt         := nvl(v_debit_value, 0) -
                               nvl(v_credit_value, 0);
  ...
  FUNCTION get_pos_entry_mode(p_docno     NUMBER,
                              p_card_no   VARCHAR2,
                              p_tran_code VARCHAR2 := NULL) RETURN NUMBER IS
  ...
  BEGIN
    ...
    IF p_docno IS NOT NULL THEN
      BEGIN
        ...
        IF v_pos_entry_mode IS NULL THEN
          BEGIN
  ...
  BEGIN
    ...
    IF p_docno IS NOT NULL THEN
      BEGIN
        ...
        IF v_pos_poscondition IS NULL THEN
          BEGIN
            SELECT DISTINCT trancode
              INTO v_trancode
              FROM textract
  ...
  BEGIN
    BEGIN
      SELECT get_pos_condition(p_docno, p_card_no, p_tran_code)
        INTO v_is_ecom
        FROM dual;
  ...
  BEGIN
    ...
    BEGIN
      ...
      BEGIN
        ...
        --CLOSE install_acc_list;
      END;
      v_install_balance_bdt := nvl(v_install_credit_bal, 0) -
                               nvl(v_install_debit_bal, 0);
      IF v_install_balance_bdt > 0 THEN
        v_install_balance_bdt := 0;
      END IF;
    
      v_balance_bdt := nvl(v_credit_value, 0) - nvl(v_debit_value, 0);
      IF v_balance_bdt > 0 THEN
        v_balance_bdt := 0;
      END IF;
  ...
  BEGIN
    BEGIN
      ...
      IF v_curr = 840 THEN
        ...
        v_balance_usd      := nvl(v_credit_value_usd, 0) -
                              nvl(v_debit_value_usd, 0);
        --dbms_output.put_line(v_balance_bdt);
        --dbms_output.put_line(v_balance_usd);
        IF v_balance_usd > 0 THEN
          v_balance_usd := 0;
    ...
    begin
      select sum(case
       ...
       where custom_transaction.get_entcode_by_doc(t.docno) = 63
         and (t.pan like ('371598%') or t.pan like ('376947%') or
              t.pan like ('376948%') or t.pan like ('371599%'))
