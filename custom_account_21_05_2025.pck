CREATE OR REPLACE PACKAGE custom_account IS
    ...
    */
	...
	FUNCTION pan_is_islamic(p_pan VARCHAR2) RETURN NUMBER;
END custom_account;
/
CREATE OR REPLACE PACKAGE BODY custom_account IS
    ...
    */
		...
		THEN
			...
			BEGIN
				...
				FROM   (SELECT t.postdate
						...
						WHERE  t.branch = 1
						AND    t.debitentcode NOT IN (79, 401, 429, 134)
	...
	FUNCTION pan_is_islamic(p_pan VARCHAR2) RETURN NUMBER IS
		v_acc_type NUMBER := 0;
	BEGIN
		BEGIN
			SELECT COUNT(a.accounttype)
			INTO   v_acc_type
			FROM   tcard t
			JOIN   tacc2card c
			ON     c.pan = t.pan
