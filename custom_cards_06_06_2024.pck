CREATE OR REPLACE PACKAGE custom_cards IS
  ...
  FUNCTION get_limit_max_value(p_pan VARCHAR2, p_limit_id NUMBER)
    RETURN VARCHAR2;

  FUNCTION get_period_type(p_pan VARCHAR2, p_limit_id NUMBER) RETURN VARCHAR2;
  FUNCTION get_period(p_pan VARCHAR2, p_limit_id NUMBER) RETURN VARCHAR2;

  FUNCTION get_card_stat_status(p_stat_id VARCHAR2, p_event_typ NUMBER)
    RETURN VARCHAR2;
  ...
  FUNCTION getcardtattributebyparamid(p_pan VARCHAR2, p_paramid VARCHAR2)
    RETURN VARCHAR2;
  FUNCTION get_card_type_frm_pan(p_pan VARCHAR2) RETURN VARCHAR2;
  FUNCTION getcardtattributebyparamiddesc(p_pan     VARCHAR2,
                                          p_paramid VARCHAR2) RETURN VARCHAR2;
...
CREATE OR REPLACE PACKAGE BODY custom_cards IS
  ...
  BEGIN
    IF p_sign_stat IS NOT NULL THEN
      SELECT rcs.name
        INTO v_state_name
  ...
  BEGIN
    ...
    RETURN v_max_value;
  END;
  FUNCTION get_period_type(p_pan VARCHAR2, p_limit_id NUMBER) RETURN VARCHAR2 IS
  ...
  FUNCTION get_period(p_pan VARCHAR2, p_limit_id NUMBER) RETURN VARCHAR2 IS
  ...
  BEGIN
    ...
    BEGIN
      IF p_event_typ = 5 THEN
        SELECT t.name
          INTO v_result
          FROM treferencecrd_stat t
         WHERE t.crd_stat = p_stat_id;
      ELSIF p_event_typ = 6 THEN
    ...
    BEGIN
      SELECT pan
        ...
        FROM (SELECT c.pan,
                ...
                FROM a4m.tcontract a, a4m.tcontractcarditem b, a4m.tcard c
               WHERE a.branch = 1
  ...
  BEGIN
    ...
    BEGIN
      IF v_contract_no IS NOT NULL THEN
        SELECT t.idclient
          INTO v_client_id
          FROM tcard t
         WHERE t.pan = p_card_no;
        IF p_card_no NOT LIKE ('78000%') THEN
    ...
    BEGIN
      FOR i IN (SELECT oap.extid,
                 ...
                 WHERE oap.extid = p_paramid
                   AND t.pan = p_pan) LOOP
        --v_contract := i.no;
        IF lower(i.ptype) = 'date' THEN
        
          v_result := i.valuedate;
        ELSIF lower(i.ptype) = 'str' THEN
          v_result := i.valuestr;
        ELSIF lower(i.ptype) = 'num' THEN
          v_result := i.valuenum;
        ELSE
          v_result := ' ';
        END IF;
  ...
  BEGIN
    ...
    END;
    BEGIN
      IF master_card = TRUE THEN
        RETURN 'MASTER CARD';
      ELSIF visa_card = TRUE THEN
        RETURN 'VISA CARD';
        /*ELSIF visa_card_1 = TRUE
        THEN
            RETURN 'VISA CARD 1';*/
      ELSIF amex_card = TRUE THEN
        RETURN 'AMEX CARD';
      ELSIF diners_card = TRUE THEN
        RETURN 'DINERS CARD';
      ELSIF discover_card = TRUE THEN
        RETURN 'DISCOVER CARD';
      ELSIF jcb_card = TRUE THEN
        RETURN 'JCB CARD';
      
        /*          ELSIF priority_pass = TRUE
        THEN
            RETURN 'PRIORITY PASS CARD';*/
      ELSIF cup_card = TRUE THEN
        RETURN 'CUP CARD';
      ELSE
        RETURN 'OTHER CARD';
      END IF;
  ...
  FUNCTION getcardtattributebyparamiddesc(p_pan     VARCHAR2,
                                          p_paramid VARCHAR2) RETURN VARCHAR2 IS
    v_result VARCHAR2(200);
    --v_contract VARCHAR2(20);
  BEGIN
  ...
  BEGIN
    BEGIN
      SELECT upper(TRIM(m.card_type))
        INTO v_card_type
        FROM (WITH binlist AS (SELECT 1 AS branch, a.*
                                 FROM a4m.custom_card_bin a), ppan AS (SELECT 1     AS branch,
                                                                              p_pan AS pan
                                                                         FROM dual)
               SELECT substr(b.pan, 1, length(a.card_bin)) bin, a.*
                 FROM binlist a
                 LEFT JOIN ppan b
      ...
      ELSE
        IF v_card_type IN ('MASTERCARD DEBIT', 'DEBIT CARDS', 'DEBIT CARD') THEN
          v_card_type := 'DEBIT';
        ELSIF v_card_type IN ('CREDIT CARD', 'MASTERCARD CREDIT') THEN
          v_card_type := 'CREDIT';
        ELSIF v_card_type = 'PREPAID CARD' THEN
          v_card_type := 'PREPAID';
        ELSIF v_card_type = 'DOMESTIC' THEN
          v_card_type := 'DOMESTIC';
        
        ELSIF v_card_type = 'VISA' THEN
          v_card_type := 'VISA';
        
        ELSIF v_card_type = 'MPAN' THEN
          v_card_type := 'MPAN';
        
        ELSIF v_card_type = 'OTHERS' THEN
          v_card_type := 'OTHERS';
        
        ELSIF v_card_type = 'ACCOUNT' THEN
          v_card_type := 'ACCOUNT';
        END IF;
      END IF;
    ...
    if p_pan is not null then
      ...
      begin
        select rr.ident
          ...
          from a4m.tcard c, a4m.tidentity i, a4m.treferenceretailer rr
         where c.branch = 1
           and c.PAN = p_pan
