CREATE OR REPLACE PACKAGE custom_account IS
	...
	/*
    Created by : Mohammad Habibur Rahman
    Created date: Development continues from 01-08-2022
    ...
    */
	...
	FUNCTION check_brpd_eli_no_tran
	(
		p_con                    VARCHAR2
	   ,p_last_positive_bal_date DATE := NULL
	) RETURN NUMBER;
	...
	FUNCTION get_credit_bal_acc(p_account_no VARCHAR2
								-- ,p_fromdate    DATE
							   ,p_to_date DATE) RETURN NUMBER;
	FUNCTION get_entry_name(p_entcode NUMBER) RETURN VARCHAR2;
	FUNCTION get_no_trans_con(p_con VARCHAR2) RETURN NUMBER;
	FUNCTION get_lst_pval_date_ftran_chk(p_con VARCHAR2) RETURN NUMBER;
	FUNCTION get_opti_bal_spec_date
	(
		p_accountno IN taccount.accountno%TYPE
	   ,p_date      IN DATE
	) RETURN NUMBER;

	FUNCTION get_acc_remain(p_acc VARCHAR2) RETURN NUMBER;

	FUNCTION get_last_positive_bal_date_bdt(p_con VARCHAR2) RETURN DATE;

	FUNCTION get_last_positive_bal_date_usd(p_con VARCHAR2) RETURN DATE;
	FUNCTION get_acc_typ_pan(p_pan VARCHAR2) RETURN NUMBER;
	FUNCTION get_last_pstv_bal_date_bdt_woff(p_con VARCHAR2) RETURN DATE;
	FUNCTION get_last_pstv_bal_date_usd_woff(p_con VARCHAR2) RETURN DATE;

	FUNCTION get_last_positive_bal_doc(p_con VARCHAR2) RETURN NUMBER;
	/*FUNCTION get_lst_pval_doc_ftran_chk(p_con VARCHAR2) RETURN NUMBER;*/
	FUNCTION get_last_positive_bal_doc
	(
		p_con VARCHAR2
	   ,p_acc VARCHAR2
	) RETURN NUMBER;
	FUNCTION get_last_pstv_bal_doc_woff
	(
		p_con VARCHAR2
	   ,p_acc VARCHAR2
	) RETURN NUMBER;
	FUNCTION check_brpd_eli_no_tran_doc
	(
		p_con                   VARCHAR2
	   ,p_last_positive_bal_doc NUMBER
	) RETURN NUMBER;
	FUNCTION check_brpd_eli_no_tran_doc_acc
	(
		p_con                   VARCHAR2
	   ,p_last_positive_bal_doc NUMBER
	   ,p_accnount              VARCHAR2
	) RETURN NUMBER;
	FUNCTION get_last_pstv_bal_date_woff
	(
		p_con     VARCHAR2
	   ,p_account VARCHAR2
	) RETURN DATE;
	FUNCTION get_ttl_transaction_count
	(
		p_con     VARCHAR2
	   ,p_account VARCHAR
	) RETURN NUMBER;
	FUNCTION get_ttl_transaction_count(p_con VARCHAR2) RETURN NUMBER;
	FUNCTION get_writof_tr_count
	(
		p_con     VARCHAR2
	   ,p_account VARCHAR
	) RETURN NUMBER;

	FUNCTION get_first_woff_doc
	(
		p_con VARCHAR2
	   ,p_acc VARCHAR2
	) RETURN NUMBER;
	FUNCTION get_last_pstv_bal_doc_bfr_1st_wf
	(
		p_con     VARCHAR2
	   ,p_account VARCHAR2
	) RETURN NUMBER;
...
CREATE OR REPLACE PACKAGE BODY custom_account IS
    ...
    Modified By: Mohammad Habibur Rahman
    Date: 27-05-2025
    Reason: New function added 'get_acc_typ_pan'
    ...
    */
	...
	FUNCTION get_last_positive_bal_date(p_con VARCHAR2) RETURN DATE IS
		v_opdate                        DATE := NULL;
		v_bdt_account                   VARCHAR2(30) := NULL;
		v_usd_account                   VARCHAR2(30) := NULL;
		v_global_limit_migrated_account VARCHAR2(30) := NULL;
	
	BEGIN
	
		BEGIN
			SELECT t.account_usd
			INTO   v_global_limit_migrated_account
			FROM   custom_global_limit t
			WHERE  t.contract_no = p_con;
		EXCEPTION
			WHEN OTHERS THEN
				v_global_limit_migrated_account := NULL;
		END;
	
		IF v_global_limit_migrated_account IS NOT NULL
		THEN
		
			BEGIN
				SELECT postdate
				INTO   v_opdate
				FROM   (SELECT t.postdate
							  ,t.docno
							  ,t.amount
							  ,SUM(CASE
									   ...
									   WHEN t.trantype IN (1, 2) THEN
										-t.amount
								   END) over(ORDER BY t.postdate ASC, t.docno ASC) cum_sum
						FROM   a4m.tcontracttrxnlist t
						JOIN   a4m.tcontractitem i
						ON     i.no = t.contractno
						AND    i.key = t.accountno
						AND    i.branch = t.branch
						WHERE  t.branch = 1
						AND    t.contractno = p_con) final_query
				WHERE  cum_sum >= 0
				ORDER  BY postdate DESC
						 ,docno    DESC
						 ,cum_sum  DESC FETCH FIRST ROW ONLY;
			EXCEPTION
				WHEN OTHERS THEN
					v_opdate := NULL;
			END;
		ELSE
			...
			EXCEPTION
				WHEN OTHERS THEN
					v_bdt_account := NULL;
			END;
			--    dbms_output.put_line('v_bdt_account ' || v_bdt_account);
			--dbms_output.put_line('v_usd_account ' || v_usd_account);
			BEGIN
				v_usd_account := get_usd_acc_by_con(p_con);
			EXCEPTION
				WHEN OTHERS THEN
			...
			IF (v_bdt_account IS NOT NULL AND v_usd_account IS NULL)
			   OR (v_usd_account IS NOT NULL AND v_bdt_account IS NULL)
			THEN
				BEGIN
					SELECT postdate
					INTO   v_opdate
			...
			END IF;
			IF v_bdt_account IS NOT NULL
			   AND v_usd_account IS NOT NULL
			THEN
			
				--dbms_output.put_line('Please use specific currency wise funtion like - get_last_positive_bal_date_bdt');
				v_opdate := '01-JUN-1991';
				RETURN v_opdate;
				/*SELECT postdate
                INTO   v_opdate
                FROM   (
                        ...
                        SELECT ttl.postdate
                               ,ttl.docno
                               ,ttl.amount
                               ,SUM(ttl.amount) over(ORDER BY ttl.postdate ASC, ttl.docno ASC) cum_sum
                        FROM   (SELECT t.postdate
                                       ,t.docno
                                       ,t.accountno
                                       ,t.rate
                                       ,t.trantype
                                       ,(CASE
                                            WHEN t.trantype IN (3, 4)
                                                 AND t.accountno = v_bdt_account THEN
                                             round(t.amount, 2)
                                            WHEN t.trantype IN (1, 2)
                                                 AND t.accountno = v_bdt_account THEN
                                             -round(t.amount, 2)
                                            WHEN t.trantype IN (3, 4)
                                                 AND t.accountno = v_usd_account THEN
                                             round(t.amount * round(t.rate, 2), 2)
                                            WHEN t.trantype IN (1, 2)
                                                 AND t.accountno = v_usd_account THEN
                                             -round(t.amount * round(t.rate, 2), 2)
                                        END) amount
                                 ...
                                 AND    i.branch = t.branch
                                 WHERE  t.branch = 1
                                 AND    t.contractno = p_con
                                 AND    (i.key = v_bdt_account OR i.key = v_usd_account)
                
                                 ORDER  BY t.postdate ASC
                                          ,t.docno    ASC) ttl
                
                        --ORDER BY ttl.postdate DESC, ttl.docno DESC
                        )
	...
	(
		p_con                    VARCHAR2
	   ,p_last_positive_bal_date DATE
	) RETURN NUMBER IS
		v_flag                   NUMBER := 0;
		v_count                  NUMBER := 0;
		v_last_positive_bal_date DATE := NULL;
	
		v_lst_pval_date_bdt         DATE := NULL;
		v_lst_pval_date_usd         DATE := NULL;
		v_bdt_account               VARCHAR2(100) := NULL;
		v_usd_account               VARCHAR2(100) := NULL;
		last_pstv_bal_date_bdt_woff DATE := NULL;
		last_pstv_bal_date_usd_woff DATE := NULL;
		writof_tr_count_bdt         NUMBER := 0;
		writof_tr_count_usd         NUMBER := 0;
	
		---Checking transaction after last positive balance date
		--1=no transaction after last positive balanace
		--0=have transaction after last positive balance
		--0 = NOT ELIGIBLE, 1= ELIGIBLE
	BEGIN
		v_last_positive_bal_date := p_last_positive_bal_date;
	
		v_bdt_account       := get_bdt_acc_by_con(p_con);
		v_usd_account       := get_usd_acc_by_con(p_con);
		writof_tr_count_bdt := get_writof_tr_count(p_con, v_bdt_account);
		writof_tr_count_usd := get_writof_tr_count(p_con, v_usd_account);
		--last_pstv_bal_date_bdt_woff := get_last_pstv_bal_date_bdt_woff(p_con);
	
		IF v_last_positive_bal_date IS NOT NULL
		   AND v_last_positive_bal_date <> '01-JUN-1991'
		THEN
			IF v_bdt_account IS NULL
			THEN
				v_last_positive_bal_date := v_last_positive_bal_date;
				IF writof_tr_count_usd > 0
				THEN
					last_pstv_bal_date_usd_woff := get_last_pstv_bal_date_usd_woff(p_con);
					IF last_pstv_bal_date_usd_woff IS NOT NULL
					THEN
						v_last_positive_bal_date := last_pstv_bal_date_usd_woff;
					END IF;
				END IF;
			ELSE
				v_last_positive_bal_date := v_last_positive_bal_date;
				IF writof_tr_count_bdt > 0
				THEN
					last_pstv_bal_date_bdt_woff := get_last_pstv_bal_date_bdt_woff(p_con);
					IF last_pstv_bal_date_bdt_woff IS NOT NULL
					THEN
						v_last_positive_bal_date := last_pstv_bal_date_bdt_woff;
					END IF;
				END IF;
			END IF;
			...
			BEGIN
				...
				WHERE  t.branch = 1
				AND    t.contractno = p_con
				AND    t.postdate >= v_last_positive_bal_date
				AND    t.debitentcode IN (467
										 ...
										 ,907
										 ,84
										 ,478 --INSTALLMENT_ACCELERATION_OFF
										  --,436 --DEBIT_ADJUST
										 ,434 --PREV BALANCE 
										  );
			EXCEPTION
			...
			ELSE
				NULL;
			END IF;
		ELSIF v_last_positive_bal_date IS NOT NULL
			  AND v_last_positive_bal_date = '01-JUN-1991'
		THEN
		
			IF v_usd_account IS NOT NULL
			THEN
				v_lst_pval_date_usd := get_last_positive_bal_date_usd(p_con);
				IF writof_tr_count_usd > 0
				THEN
					last_pstv_bal_date_usd_woff := get_last_pstv_bal_date_usd_woff(p_con);
					IF last_pstv_bal_date_usd_woff IS NOT NULL
					THEN
						v_lst_pval_date_usd := last_pstv_bal_date_usd_woff;
					END IF;
				END IF;
			END IF;
			IF v_bdt_account IS NOT NULL
			THEN
				v_lst_pval_date_bdt := get_last_positive_bal_date_bdt(p_con);
				IF writof_tr_count_bdt > 0
				THEN
					last_pstv_bal_date_bdt_woff := get_last_pstv_bal_date_bdt_woff(p_con);
					IF last_pstv_bal_date_bdt_woff IS NOT NULL
					THEN
						v_lst_pval_date_bdt := last_pstv_bal_date_bdt_woff;
					END IF;
				END IF;
			END IF;
			...
			BEGIN
				...
				WHERE  t.branch = 1
				AND    t.contractno = p_con
				AND    t.accountno = v_bdt_account
				AND    t.postdate >= v_lst_pval_date_bdt
				AND    t.debitentcode IN (467
										 ,435
										 ,408
										 ,135
			...
			ELSE
				...
				BEGIN
					...
					WHERE  t.branch = 1
					AND    t.contractno = p_con
					AND    t.accountno = v_usd_account
					AND    t.postdate >= v_lst_pval_date_usd
					AND    t.debitentcode IN (467
											 ,435
											 ,408
											 ,135
	...
	FUNCTION get_entry_name(p_entcode NUMBER) RETURN VARCHAR2 IS
		v_entry_name VARCHAR2(100) := '';
	BEGIN
		BEGIN
			SELECT t.name
			INTO   v_entry_name
			FROM   treferenceentry t
			WHERE  t.code = p_entcode
			AND    t.branch = 1;
		EXCEPTION
			WHEN OTHERS THEN
				v_entry_name := '';
	...
	FUNCTION get_no_trans_con(p_con VARCHAR2) RETURN NUMBER IS
		v_flag  NUMBER := 0;
		v_count NUMBER := 0;
		--Checking starter/non-started 1=non starter, 0=starter
		--0 = NOT ELIGIBLE, 1= ELIGIBLE
	BEGIN
		BEGIN
			SELECT COUNT(*)
	...
	FUNCTION get_lst_pval_date_ftran_chk(p_con VARCHAR2) RETURN NUMBER IS
		v_lst_pval_date     DATE := NULL;
		v_lst_pval_date_bdt DATE := NULL;
		v_lst_pval_date_usd DATE := NULL;
		v_bdt_account       VARCHAR2(100) := NULL;
		v_usd_account       VARCHAR2(100) := NULL;
		...
		v_count NUMBER := 0;
		---return, if 1 then need to exclude else need to include
	BEGIN
		BEGIN
			v_lst_pval_date := get_last_positive_bal_date(p_con);
		EXCEPTION
			WHEN OTHERS THEN
				v_lst_pval_date := NULL;
				NULL;
		END;
	
		IF v_lst_pval_date IS NOT NULL
		   AND v_lst_pval_date = '01-JUN-1991'
		THEN
			v_lst_pval_date_bdt := get_last_positive_bal_date_bdt(p_con);
			v_lst_pval_date_usd := get_last_positive_bal_date_usd(p_con);
			v_bdt_account       := get_bdt_acc_by_con(p_con);
			...
			BEGIN
				...
				AND    t.postdate = v_lst_pval_date_bdt
				AND    t.accountno = v_bdt_account
				AND    t.debitentcode IN
					  --   (999, 1009, 1011, 1012, 1013, 1000, 1003, 1004, 1005, 1006, 1007, 998);
					   (21
						,811
						,999
						,1009
		...
		ELSE
			...
			BEGIN
				...
				WHERE  t.branch = 1
				AND    t.contractno = p_con
				AND    t.postdate = v_lst_pval_date
				AND    t.debitentcode IN
					  --   (999, 1009, 1011, 1012, 1013, 1000, 1003, 1004, 1005, 1006, 1007, 998);
					   (21
	...
	BEGIN
		...
		FROM   (SELECT -a.value VALUE
				...
				FROM   tentry    a
					  ,tdocument b
				WHERE  a.branch = vbranch
				AND    a.debitaccount = p_accountno
				AND    b.branch = a.branch
				AND    b.docno = a.docno
				AND    b.newdocno IS NULL
				UNION ALL
				...
				FROM   tentry    a
					  ,tdocument b
				WHERE  a.branch = vbranch
				AND    a.creditaccount = p_accountno
				AND    b.branch = a.branch
				AND    b.docno = a.docno
				AND    b.newdocno IS NULL) u
		WHERE  u.opdate >= p_date;
	...
	FUNCTION get_acc_remain(p_acc VARCHAR2) RETURN NUMBER IS
		vacchandle NUMBER;
		vremain    NUMBER;
	BEGIN
		vacchandle := account.newobject(p_acc, 'R');
		vremain    := account.getremain(vacchandle);
		RETURN vremain;
	END;

	FUNCTION get_last_positive_bal_date_bdt(p_con VARCHAR2) RETURN DATE IS
		v_opdate      DATE := NULL;
		v_bdt_account VARCHAR2(30) := NULL;
		--v_usd_account                   VARCHAR2(30) := NULL;
		--v_global_limit_migrated_account VARCHAR2(30) := NULL;
		...
		THEN
			BEGIN
				...
				FROM   (SELECT t.postdate
						...
						WHERE  t.branch = 1
						AND    t.contractno = p_con
						AND    i.key = v_bdt_account) final_query
				WHERE  cum_sum >= 0
				ORDER  BY postdate DESC
						 ,docno    DESC
						 ,cum_sum  DESC FETCH FIRST ROW ONLY;
	...
	FUNCTION get_last_positive_bal_date_usd(p_con VARCHAR2) RETURN DATE IS
		v_opdate DATE := NULL;
		--v_bdt_account                   VARCHAR2(30) := NULL;
		v_usd_account VARCHAR2(30) := NULL;
		--v_global_limit_migrated_account VARCHAR2(30) := NULL;
		...
		THEN
			BEGIN
				...
				FROM   (SELECT t.postdate
						...
						WHERE  t.branch = 1
						AND    t.contractno = p_con
						AND    i.key = v_usd_account) final_query
				WHERE  cum_sum >= 0
				ORDER  BY postdate DESC
						 ,docno    DESC
						 ,cum_sum  DESC FETCH FIRST ROW ONLY;
	...
	FUNCTION get_acc_typ_pan(p_pan VARCHAR2) RETURN NUMBER IS
	...
	FUNCTION get_last_pstv_bal_date_bdt_woff(p_con VARCHAR2) RETURN DATE IS
		v_opdate      DATE := NULL;
		v_bdt_account VARCHAR2(30) := NULL;
		--v_usd_account                   VARCHAR2(30) := NULL;
		v_global_limit_migrated_account VARCHAR2(30) := NULL;
		...
		THEN
			BEGIN
				...
				FROM   (SELECT t.postdate
						...
						WHERE  t.branch = 1
						AND    t.contractno = p_con
						AND    t.debitentcode NOT IN (134, 438)
						AND    i.key = v_bdt_account) final_query
				WHERE  cum_sum >= 0
				ORDER  BY postdate DESC
						 ,docno    DESC
	...
	FUNCTION get_last_pstv_bal_date_usd_woff(p_con VARCHAR2) RETURN DATE IS
		v_opdate DATE := NULL;
		--v_bdt_account                   VARCHAR2(30) := NULL;
		v_usd_account                   VARCHAR2(30) := NULL;
		v_global_limit_migrated_account VARCHAR2(30) := NULL;
	...
	FUNCTION get_last_positive_bal_doc(p_con VARCHAR2) RETURN NUMBER IS
		v_docno                         NUMBER := NULL;
		v_bdt_account                   VARCHAR2(30) := NULL;
		v_usd_account                   VARCHAR2(30) := NULL;
		v_global_limit_migrated_account VARCHAR2(30) := NULL;
			...
			BEGIN
				...
				FROM   (SELECT t.docno
							  ...
							  ,SUM(CASE
									   ...
									   WHEN t.trantype IN (1, 2) THEN
										-t.amount
								   END) over(ORDER BY t.docno ASC, t.postdate ASC) cum_sum
						FROM   a4m.tcontracttrxnlist t
						WHERE  t.branch = 1
						AND    t.contractno = p_con) final_query
				WHERE  cum_sum >= 0
	...
	(
	   ...
	   ,p_last_positive_bal_doc NUMBER
		...
		v_flag                  NUMBER := 0;
		v_count                 NUMBER := 0;
		v_last_positive_bal_doc NUMBER := NULL;
	
		v_lst_pval_doc_bdt NUMBER := NULL;
		v_lst_pval_doc_usd NUMBER := NULL;
		v_bdt_account      VARCHAR2(100) := NULL;
		v_usd_account      VARCHAR2(100) := NULL;
		---Checking transaction after last positive balance date
		--1=no transaction after last positive balanace
		...
		--0 = NOT ELIGIBLE, 1= ELIGIBLE
	BEGIN
		v_last_positive_bal_doc := p_last_positive_bal_doc;
		IF v_last_positive_bal_doc IS NOT NULL
		   AND v_last_positive_bal_doc <> 01061991
		THEN
			BEGIN
				SELECT COUNT(*)
				INTO   v_count
				...
				WHERE  t.branch = 1
				AND    t.contractno = p_con
				AND    t.docno > v_last_positive_bal_doc
				AND    t.debitentcode IN (467
										 ,435
										 ,408
										 ,135
			...
			ELSE
				NULL;
			END IF;
		ELSIF v_last_positive_bal_doc IS NOT NULL
			  AND v_last_positive_bal_doc = 01061991
		THEN
			v_flag := 0;
			RETURN v_flag;
	...
	(
	   ...
	   ,p_accnount              VARCHAR2
		...
		v_count                 NUMBER := 0;
		v_last_positive_bal_doc NUMBER := NULL;
		last_pstv_bal_date_woff DATE := NULL;
		v_writof_tr_count       NUMBER := 0;
		---Checking transaction after last positive balance date
		--1=no transaction after last positive balanace
		--0=have transaction after last positive balance
		--0 = NOT ELIGIBLE, 1= ELIGIBLE
	BEGIN
		v_last_positive_bal_doc := p_last_positive_bal_doc;
		v_writof_tr_count       := get_writof_tr_count(p_con, p_accnount);
		IF v_writof_tr_count > 0
		THEN
			last_pstv_bal_date_woff := get_last_pstv_bal_date_woff(p_con, p_accnount);
		END IF;
		IF v_last_positive_bal_doc IS NOT NULL
		   AND p_accnount IS NOT NULL
		THEN
			BEGIN
				SELECT COUNT(*)
				INTO   v_count
				...
				WHERE  t.branch = 1
				AND    t.contractno = p_con
				AND    t.accountno = p_accnount
				AND    (t.docno > v_last_positive_bal_doc OR (last_pstv_bal_date_woff IS NOT NULL AND
					  t.postdate > last_pstv_bal_date_woff))
				AND    t.debitentcode IN (467
										 ,435
										 ,408
										 ,135
	...
	FUNCTION get_ttl_transaction_count(p_con VARCHAR2) RETURN NUMBER IS
		v_count NUMBER := 0;
	BEGIN
		BEGIN
			SELECT COUNT(*)
	...
	BEGIN
		BEGIN
			...
			ORDER  BY postdate ASC
					 ,docno    ASC FETCH FIRST ROW ONLY;
		EXCEPTION
			WHEN OTHERS THEN
				v_docno := NULL;
		END;
	...
	(
	   ...
	   ,p_account VARCHAR2
		...
		v_docno          NUMBER := NULL;
		v_first_woff_doc NUMBER := NULL;
	
	BEGIN
		v_first_woff_doc := get_first_woff_doc(p_con, p_account);
		BEGIN
			SELECT docno
			INTO   v_docno
			FROM   (SELECT t.postdate
					...
					AND    i.branch = t.branch
					WHERE  t.branch = 1
					AND    t.docno < v_first_woff_doc
						  --AND    t.debitentcode NOT IN (134, 438)
					AND    t.contractno = p_con
					AND    t.accountno = p_account) final_query
			WHERE  cum_sum >= 0
			ORDER  BY postdate DESC
					 ,docno    DESC
					 ,cum_sum  DESC FETCH FIRST ROW ONLY;
